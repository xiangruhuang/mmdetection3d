...
from mmdet3d.models.backbones import BasePointNet
from ..builder import PTS_ENCODERS
from mmdet3d.ops import three_nn, three_interpolate

@PTS_ENCODERS.register_module()
class PointNet2Encoder(BasePointNet):
    ...
    def forward(self, points):
        ...
        dists, indices = three_nn(sa_xyz[0], sa_xyz[self.num_sa - self.num_fp])
        weights = (-(dists ** 2) / (2*0.5**2)).exp()
        interpolated_features = three_interpolate(fp_features[-1], indices, weights)
        #fp_features.append(interpolated_features)
