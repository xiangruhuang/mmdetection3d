...
class CenterPointGeom(MVXTwoStageDetector):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None,
                 prevoxel_pts_layer=None):
        super(CenterPointGeom,
              self).__init__(pts_voxel_layer, pts_voxel_encoder,
                             pts_middle_encoder, pts_fusion_layer,
                             img_backbone, pts_backbone, img_neck, pts_neck,
                             pts_bbox_head, img_roi_head, img_rpn_head,
                             train_cfg, test_cfg, pretrained)
        if prevoxel_pts_layer:
            self.prevoxel_pts_layer = builder.build_pts_encoder(prevoxel_pts_layer)
    ...
    def extract_pts_feat(self, pts, img_feats, img_metas):
        ...
        pts_feats = []
        for p in pts:
            p_feats = self.prevoxel_pts_layer(p).squeeze(0).transpose(0, 1).contiguous()
            p_feats = torch.cat([p[:, :3], p_feats], dim=1)
            pts_feats.append(p_feats)
        coors = self.voxelize(pts_feats)
        pts_batch = torch.cat(pts_feats, dim=0)
        voxel_features, voxel_coors = self.pts_voxel_encoder(pts_batch, coors)
        batch_size = coors[-1, 0] + 1
        x = self.pts_middle_encoder(voxel_features, voxel_coors, batch_size)
